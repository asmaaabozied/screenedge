                        ...
                        <table class="table table-hover" id="table">
                            ...
                            @foreach ($types as $index=>$type)
                                <tr>
                                    ...
                                    <td>{{ $lang=='ar'? $type->title_ar :$type->title_en}}</td>

                                    <td>{{ $lang=='ar'? Str::limit($type['short_description_ar'],50) :Str::limit($type['short_description_en'],50)}}</td>
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                @if (auth()->user()->hasPermission('update_pages'))
                                                    <a href="{{ route('dashboard.pages.edit', $type->id) }}"
                                                ...
                                                <br>
                                                @if (auth()->user()->hasPermission('update_pages'))
                                                    <form action="{{ route('dashboard.pages.block', $type->id) }}"
                                                ...
                                                @if (auth()->user()->hasPermission('delete_pages'))
                                                    <form
                                                        action="{{ route('dashboard.pages.destroy', $type->id) }}"
                                                        method="post" style="display: inline-block">
                                                        {{ csrf_field() }}
                                                        {{ method_field('delete') }}
